...
from mmcv.parallel import DataContainer as DC # remove radar padding so this is needed for batch collate
...
from plugin.lirafusion.datasets.radar_points import RadarPoints
@PIPELINES.register_module()
class LoadRadarPointsMultiSweeps_lirafusion(object):
    """Load radar points from multiple sweeps.
    This is usually used for nuScenes dataset to utilize previous sweeps.
    Args:
        sweeps_num (int): Number of sweeps. Defaults to 10.
    ...
    def __call__(self, results):
        ...
        mask = (points[:, -2]) < 2 * (points[:,-3] == 0)
        points = points[mask][:,[0, 1, 2, 3, 4, 5, 8]]

        # following https://ieeexplore.ieee.org/stamp/stamp.jsp?tp=&arnumber=9483647 set z to 1
        points[:, 2] = 1 # now the radar points are Nx7
        ...
        # points = np.concatenate((points, mask), axis=-1)
        # points = DC(points.astype(np.float32))
        # points = DC(torch.from_numpy(points).to(torch.float32)) # now no more masks in the last channel for padding

        # results['radar'] = points.astype(np.float32)
        radar_points = RadarPoints(torch.from_numpy(points).to(torch.float32), points_dim=7)
        results['radar'] = radar_points
        # results['radar'] = torch.from_numpy(points).to(torch.float32)
...
@PIPELINES.register_module()
class fuse_lidar_radar(object):
    """Fuse lidar and radar points.
    """

    def __init__(self):
        self.msg = 'fuse_lidar_radar'
        ...
        Returns:
            dict: The result dict containing the multi-sweep points data. \
                ...
                - points (np.ndarray | :obj:`BasePoints`): Multi-sweep point \
                    cloud arrays.
        """
        lidar_points = results['points'].data
        new_lidar_points = torch.cat((lidar_points, torch.zeros(lidar_points.shape[0], 4).to(lidar_points.device), -torch.ones(lidar_points.shape[0], 1).to(lidar_points.device)), dim=1) # final col is the modality mask
        if 'radar' in results:
            radar_points = results['radar'].tensor
            assert radar_points.shape[1] == 7, 'radar points should be 7 dim' # update this already
            new_radar_points = torch.cat((radar_points[:,:3], torch.zeros(radar_points.shape[0], 2).to(radar_points.device), radar_points[:,3:], torch.ones(radar_points.shape[0], 1).to(radar_points.device)), dim=1) # final col is the modality mask
            # now these two points are in the same shape
            # for lidar: x,y,z,internsity,dt,0,0,0,0,-1
            # for radar: x,y,z,0,0,rcs,vx_comp,vy_comp,dt,1
            new_points = torch.cat((new_lidar_points, new_radar_points), dim=0)
        else:
            new_points = new_lidar_points
        assert new_points.shape[1] == 10, 'new points should be 10 dim'
        results['fused_points'] = DC(new_points)
        return results
...
@PIPELINES.register_module()
class Radar_format(object):
    """
    Get radar points to new radar points.
    """

    def __init__(self):
        self.msg = 'Radar_format'
        ...
        Returns:
            dict: The result dict containing the multi-sweep points data. \
                ...
                - points (np.ndarray | :obj:`BasePoints`): Multi-sweep point \
                    cloud arrays.
        """
        # Get tensor of augmented radar data into results dict
        radar_new_points = results['radar'].tensor
        # print('radar_new_points shape: ', radar_new_points.shape) # 2 dim
        results['radar'] = DC(radar_new_points)
        # results['radar'] = radar_new_points
        return results
