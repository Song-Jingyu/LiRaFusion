...
class FUTR3DCrossAtten(BaseModule):
    ...
    def __init__(self,
        ...
        if (self.use_Cam and self.use_LiDAR) or (self.use_Cam and self.use_Radar):
            self.modality_fusion_layer = nn.Sequential(
                nn.Linear(self.fused_embed, self.embed_dims),
                nn.LayerNorm(self.embed_dims),
                nn.ReLU(inplace=False),
    ...
    def forward(self,
        ...
        #     # change to (num_query, bs, radar_dim)
        #     radar_out = radar_out.permute(1, 0, 2)
        
        #     # radar_out shape: (num_query, bs, radar_dim)
        #     radar_out = self.radar_output_proj(radar_out)

        """
        Original
        """
        # if self.use_Cam and self.use_LiDAR and self.use_Radar:
        ...
        Modified V2
        """
        if self.use_Cam and (self.use_LiDAR or self.use_Radar): # camera + (lidar or radar)
            # print("img_output shape: ", img_output.shape) # (600, 1, 256)
            # print("pts_output shape: ", pts_output.shape) # (600, 1, 256)
            output = torch.cat((img_output, pts_output), dim=2).permute(1, 0, 2)
            # print("output shape: ", output.shape) # (1, 600, 512)
            output = self.modality_fusion_layer(output).permute(1, 0, 2)
        elif self.use_Cam: # camera only
            output = img_output
        elif self.use_LiDAR or self.use_Radar: # not camera + (lidar or radar)
            output = pts_output
        
        reference_points_3d = reference_points.clone()

        # print("output shape: ", output.shape) # (600, 1, 256)
        # print("self.pos_encoder(inverse_sigmoid(reference_points_3d)).permute(1, 0, 2) shape: ", self.pos_encoder(inverse_sigmoid(reference_points_3d)).permute(1, 0, 2).shape)
        # print("inp_residual shape: ", inp_residual.shape)

        # (num_query, bs, embed_dims)
        test_output = self.dropout(output) + inp_residual + self.pos_encoder(inverse_sigmoid(reference_points_3d)).permute(1, 0, 2)
        # print("test_output shape: ", test_output.shape) # (600, 600, 256) not correct !!
...
def feature_sampling_3D(mlvl_feats, reference_points, pc_range):
    ...
    num_points = 1
    for lvl, feat in enumerate(mlvl_feats):
        B, N, C, H, W = feat.size() # This is the output of FPNV2 layer!!
        feat = feat.view(B*N, C, H, W)
